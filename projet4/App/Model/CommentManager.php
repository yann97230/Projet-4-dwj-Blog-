...
Class CommentManager extends ManagerData {
	...
	public function getComments($postId) {
		$commentaire = [];
		$req = $this->dbConnect()->prepare('SELECT id, auteur, commentaire, id_billet, report, DATE_FORMAT(date_commentaire, \'%d/%m/%Y à %Hh%imin%ss\') AS date_commentaire FROM commentaires WHERE id_billet = ? ORDER BY id DESC');
		$req->execute([
            $postId
        ]);
