    ...
    <div class="contenairPost">
        ...
        <?php if (isset($_SESSION['admin'])): ?>
            <div class="bouton-Article">
                <button class="btn btn-info"><a href="index.php?action=viewArticle&amp;postId=<?= $article->getId() ?>">Modifier l'article</a></button>
                <button id="delete" class="btn btn-danger"><a href="index.php?action=delete&amp;postId=<?= $article->getId() ?>">supprimer l'article</a></button>
            </div>
 ...
 <div class="contenair">    
    <div class="backgroundComment">
       ...
       <?php if ($tab["report"] === "1"): ?>
        ...
        <?php else: ?> 
           <div class ="bouton-Article">
                <button class="btn btn-info"><a href="index.php?action=report&amp;id=<?= $commentaire->getId(); ?>&amp;post=<?= $commentaire->getId_billet(); ?>">Signaler un commentaire</a></button>
           </div>
        <?php endif; ?>

        <?php if (isset($_SESSION['admin'])): ?>
             <div class ="bouton-Article"><button class="btn btn-info"><a href="index.php?action=deleteComment&amp;id=<?= $commentaire->getId(); ?>">Supprimer le commentaire</a></button>
             </div>
        <?php endif; ?>
    </div>
</div>        
